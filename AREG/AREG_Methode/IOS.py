...
class Auto_IOS(Methode):
    ...
    def TestScan(self, scan_folder_t1: str,scan_folder_t2 : str):
        ...
        if len(super().search(scan_folder_t1,'.vtk')['.vtk']) != len(super().search(scan_folder_t1,'.vtk')['.vtk']):
    ...
    def Process(self, **kwargs):
        ...
        number_scan_toseg_T1 = self.__BypassCrownseg__(kwargs['input_t1_folder'],path_input_T1,path_seg_T1)
        number_scan_toseg_T2 = self.__BypassCrownseg__(kwargs['input_t2_folder'],path_input_T2,path_seg_T2)

        parameter_segteeth_T1 = {'input':path_input_T1,
                        'output':path_seg_T1,
                        'subdivision_level':2,
                        'resolution':320,
        ...
        parameter_segteeth_T2 = {'input':path_input_T2,
                    'output':path_seg_T2,
                    'subdivision_level':2,
                    'resolution':320,
                    'model':self.getModel(kwargs['model_folder_1'],extension='pth'),
        ...
        parameter_pre_aso_T1= {'input':path_seg_T1,
        ...
        parameter_pre_aso_T2 = {'input':path_seg_T2,
                        'gold_folder':kwargs['model_folder_2'],
        ...
        numberscan = self.NumberScan(kwargs['input_t1_folder'],kwargs['input_t2_folder'])
        list_process = [{'Process':SegProcess,'Parameter':parameter_segteeth_T1,'Module':'CrownSegmentationcli T1','Display':DisplayCrownSeg(number_scan_toseg_T1,kwargs['logPath'],'T1 Scan')},
                        {'Process':SegProcess,'Parameter':parameter_segteeth_T2,'Module':'CrownSegmentationcli T2','Display':DisplayCrownSeg(number_scan_toseg_T2,kwargs['logPath'],'T2 Scan')},
                        {'Process':PreOrientProcess,'Parameter':parameter_pre_aso_T1,'Module':'PRE_ASO_IOS T1','Display':DisplayASOIOS(numberscan,kwargs['logPath'],'T1 Patient')},
                        {'Process':PreOrientProcess,'Parameter':parameter_pre_aso_T2,'Module':'PRE_ASO_IOS T2','Display':DisplayASOIOS(numberscan,kwargs['logPath'],'T2 Patient')},
                        {'Process':RegProcess,'Parameter':parameter_reg,'Module':'AREG_IOS','Display':DisplayAREGIOS(numberscan,kwargs['logPath'])}
        ]
...
class Semi_IOS(Auto_IOS):
    ...
    def Process(self, **kwargs):
        ...
        RegProcess = slicer.modules.areg_ios
        numberscan = self.NumberScan(kwargs['input_t1_folder'],kwargs['input_t2_folder'])
        processus= [{'Process':RegProcess,'Parameter':parameter_reg,'Module':'AREG_IOS','Display':DisplayAREGIOS(numberscan,kwargs['logPath'])}]
        return processus
