...
def createCondaEnv(name:str,default_install_path:str,path_conda:str,path_activate:str) :
      ...
      f"source {path_activate} {name} && pip install scipy"
      ]
...
def call(default_install_path,args,name):
    ...
    command = f"source {activate_env} {name} && {python_executable} {path_server}"

    # Start server
    server_process = subprocess.Popen(command, shell=Tru)
